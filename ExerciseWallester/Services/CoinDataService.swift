...
class CoinDataService {
    ...
    func fetchCryptoCurrencyDetails(id: String, forceUpdate: Bool = false) -> AnyPublisher<CoinDetailModel, Error> {
        let urlWithID = "https://api.coingecko.com/api/v3/coins/\(id)?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=true"
    ...
    func fetchCryptoCurrencyHistoricalChartData(id: String, currency: String, timeframe: String, forceUpdate: Bool = false) -> AnyPublisher<CoinHistoricalChartDataModel, Error> {
